...
public class CopyAndPasteFieldsTestScripts extends LaunchBrowser {
	...
	public void copyAndPasteFieldsIntoNewForm(ScriptPage scriptPage) {
		...
		scriptPage.clickOnCopyMultipleFields().waitForSaveOrPublishNotification();
		...
		scriptPage.clickOnSaveScriptButton().clickOnOutputOrderNotificationPopUp().waitForAccountLoadOrSave()
				.waitForSaveOrPublishNotification();
		scriptPage.clickOnFormbutton(1);
	}

	public void copyAndPasteDecisionTreeIntoNewForm(ScriptPage scriptPage) {
		scriptPage.clickOnDecisionTree().clickOnCopyDecisionTree().waitForSaveOrPublishNotification();
	...
	public void validatingEditedFieldsInFc(ScriptPage scriptPage) {
		...
		getReport().log(Status.PASS, "Copy And Paste Of Fields Are Successful In FC");
	}
	...
	public void validatingScriptInSb() {
		...
		getReport().log(Status.PASS, "Copy Pasted Fields Loaded Sucessfully In Sb");
	...
	@Test
	public void verifyCopyPasteFields(String accountNumber) {
		report = extent.createTest("Verify Copy And Paste Fields");
		setReport(report);
		...
		copyAndPasteFieldsIntoNewForm(scriptPage);
		copyAndPasteDecisionTreeIntoNewForm(scriptPage);
